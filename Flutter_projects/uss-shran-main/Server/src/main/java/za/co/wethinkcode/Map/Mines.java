...
public class Mines implements Obstacle{

    private final int bottomLeftX;
    private final int bottomLeftY;

    /**
     * Constructor for the Mines class, sets the positions
     * @param x: position x;
     * @param y: position y;
     * */
    ...
    public boolean blocksPosition(Position position) {
        return (position.getX() >= bottomLeftX && position.getX() <= (bottomLeftX + getSize())) &&
                (position.getY() >= bottomLeftY && position.getY() <= (bottomLeftY + getSize()));
    }
